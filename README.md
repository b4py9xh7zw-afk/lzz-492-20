...
面向蓝领招聘排班场景的工伤闭环管理：**现场主管手机端即时上报 → 平台自动核对缺件 → 劳务公司补保险资料 → 企业只读查看本项目 → 复工/停工结论回写排班档案**。

### 角色与权限

| 角色 | 测试账号 | 数据范围 | 关键操作 |
| --- | --- | --- | --- |
| 平台管理员 | `admin / 123456` | 全部项目 | 全部操作 |
| 现场主管 | `supervisor1 / 123456` | 仅本项目 | 手机端上报、传现场证明、下复工/停工结论 |
| 劳务公司 | `labor1 / 123456` | 本公司合作项目 | 补充保险资料、上传合同/保单/身份证、补传票据 |
| 用工企业 | `enterprise1 / 123456` | **仅本项目，全程只读** | 查看上报、材料、保险、结论与排班留痕 |

> 鉴权方式：登录后前端在所有请求头携带 `X-User-Id`，后端 `LoginInterceptor` 解析为 `CurrentUserHolder`，
> 所有查询/详情/写操作在 Service 层按角色强制做项目隔离，越权返回 403。

### 业务流程

1. **主管手机上报**（H5「工伤」→ 现场工伤上报）
   - 从近 14 天排班档案带出 **项目 / 工人 / 岗位 / 班次**，补录也能选到历史排班；
   - 记录 **受伤时间、地点、经过、伤情类型、送医医院/时间**，支持多名 **见证人**（姓名/电话/身份/说明）；
   - 可「暂存草稿」或「提交上报」；提交即把工伤单挂到排班档案并生成首条档案事件。
2. **平台缺件提示**（详情页材料清单）
   - 按《工伤保险条例》第十八条等要求内置材料目录，按伤情类型（门诊/住院/疑似伤残/工亡）动态生成必需清单；
   - 每项材料区分责任方：**现场主管**（现场照片、见证人证言、事故报告）、**劳务公司**（劳动合同、参保证明/保单、身份证、伤残鉴定等）、**医疗票据**（诊断证明、病历、费用票据）；
   - 上传/删除材料实时重算完成度，顶部 NoticeBar 直接提示「还缺哪几份、由谁补」；材料 100% 时单据状态自动变为「材料齐」。
3. **劳务补充保险资料**（H5/PC 详情页「保险资料」，仅劳务可编辑）
   - 承保机构、保单号、保障期间、理赔状态（未报案/已报案/理赔中/已赔付/拒赔）、报案号、经办人等。
4. **企业只读查看**：企业账号看不到其他项目数据，上传/保存/结论接口对其返回 403。
5. **复工/停工结论回写排班档案**（主管确认）
   - 停工：写停工开始日、预计复工日、医嘱；复工：写实际复工日与复工条件；
   - 结论回写 `schedule` 档案状态（`injury_stop / resumed`），并在 `schedule_event` 留痕；
   - H5「排班」页与 PC「排班档案」可按日期查看状态与完整时间线，工伤单一键跳转。

### 材料清单规则（后端 `MaterialCatalog`）

- 通用必需：事故现场照片、见证人证言、事故经过报告、医疗诊断证明、医疗费用票据、劳动合同/用工关系证明、参保证明/保单、身份证复印件；
- 门诊：加门诊病历；住院：加住院病历/出院小结、支付凭证；疑似伤残：再加劳动能力鉴定；工亡：加死亡证明、供养亲属证明。

### 新增/修改的数据表

| 表 | 说明 |
| --- | --- |
| `user`（扩展） | 新增 `role / phone / project_id / company_name` |
| `project` | 项目（企业、劳务公司、地址） |
| `worker` | 工人（归属劳务公司） |
| `schedule` | 排班档案，含 `schedule_status / injury_report_id / stop_start_date / resume_date / conclusion` |
| `injury_report` | 工伤上报主单 |
| `injury_witness` | 见证人 |
| `injury_material` | 证明材料（类型、责任方、文件） |
| `injury_insurance` | 劳务补充的保险资料（一单一档） |
| `schedule_event` | 排班档案事件流（上报/停工/复工） |

- 全新环境：`database/init.sql` 已包含全部表结构与演示数据（Docker 初始化自动执行）；
- 已有数据库：执行幂等升级脚本 `database/injury_upgrade.sql`（可重复执行）；
- 仅工伤模块演示数据：`database/injury.sql`。

### 主要接口（前缀 `/api`）

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/injury/report` | 主管新建/暂存/提交工伤上报 |
| GET | `/injury/page` | 分页查询（自动按角色隔离） |
| GET | `/injury/{id}` | 详情（含缺件清单/保险/档案事件/权限位） |
| GET | `/injury/{id}/checklist` | 材料缺件核对 |
| POST | `/injury/material/upload` | 上传材料（图片/PDF，按角色校验责任方） |
| DELETE | `/injury/material/{id}` | 删除材料并重算完成度 |
| GET | `/injury/material/preview/{id}` | 在线预览材料 |
| POST | `/injury/{id}/insurance` | 劳务补充保险资料 |
| POST | `/injury/{id}/conclusion` | 复工/停工结论（回写排班档案） |
| GET | `/schedule/projects` `/workers` `/list` `/{id}/events` | 项目/工人/排班档案/事件 |

### 测试

- 后端自带 13 个集成测试（H2 MySQL 模式，无需外部数据库）：`mvn test`
  - 覆盖：主管提交并回写档案、缺件清单计算、企业只读 403、跨项目隔离、劳务补保险、材料进度、停工→复工时间线、HTTP 头鉴权与材料上传。
- 前端：`npm run build` 通过；PC（Element Plus）与 H5（Vant）双端均提供页面入口。
